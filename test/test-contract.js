...
describe("Contract Tests", function () {
  it("Test the contract life circle main path", async function () {
    ...
    try {
      ...
      var txs = await account_shareholder.sendTransaction({to:offer.target, value:30000000000000001n});
      ...
      txs = await account_shareholder.sendTransaction({to:offer.target, value:10000000000000001n});
  ...
  it("Test the case without observers", async function () {
    ...
    try {
      ...
      {
        var txs = await account_shareholder2.sendTransaction({to:offer.target, value: 30000000000000001n});
        var txs_receipt = await txs.wait();
      }
      {
        var end_balance_shareholder = await account_shareholder.provider.getBalance(account_shareholder.address);
      ...
      {
        var txsv = await shareholder2_access.share_vote(account_contractor.address);
        var txsv_receipt = await txsv.wait();
      }
  ...
  it("Test the voting start balance prevents early contract finishing", async function () {
    ...
    try {
      ...
      {
        var txs = await account_shareholder.sendTransaction({to:offer.target, value:10000000000000001n});
        var txs_receipt = await txs.wait();
      }
      {
        var end_balance_shareholder = await account_shareholder.provider.getBalance(account_shareholder.address);
      ...
      o.interface.parseError(
          // try ... catch(e) { parseError(e.data) ...
          (await shareholder_access.share_vote(account_contractor.address).should.eventually.rejectedWith('reverted')).data
      ).name.should.be.equal('ShareBalanceLow');
      {
        var state = await contractor_access.state();
  ...
  it("Test the voting start count prevents early contract finishing", async function () {
    ...
    try {
      ...
      o.interface.parseError(
          // try ... catch(e) { parseError(e.data) ...
          (await shareholder_access.share_vote(account_contractor.address).should.eventually.rejectedWith('reverted')).data
      ).name.should.be.equal('ShareCountLow');
      {
        var state = await contractor_access.state();
        console.log('State should not be changed', state);
        state.should.be.equal(1n);
  ...
  it("Test the voting start timeout fails the offer", async function () {
    ...
    try {
      ...
      {
        var txac = await outside_access.calculate_voting();
        var txac_receipt = await txac.wait();
      }
      ...
      {
        var txs = await shareholder_access.share_cancel();
        var txs_receipt = await txs.wait();
      }
      {
        var end_balance_shareholder = await account_shareholder.provider.getBalance(account_shareholder.address);
  ...
  it("Test the voting for failure", async function () {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, 1000));
      {
        var txsv = await shareholder_access.share_vote_failure();
        var txsv_receipt = await txsv.wait();
      }
      await new Promise(resolve => setTimeout(resolve, 1000));
      {
      ...
      {
        var txov = await observer_access.observer_vote_failure();
        var txov_receipt = await txov.wait();
      }
      await new Promise(resolve => setTimeout(resolve, 1000));
      {
